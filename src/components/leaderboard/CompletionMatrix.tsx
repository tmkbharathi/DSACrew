    ...
    <div className="glass-panel bg-slate-900/80 border border-slate-800 rounded-2xl p-4 sm:p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Grid className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-400" />
          <h3 className="font-bold text-base sm:text-lg text-white">Daily Problem Completion Grid</h3>
        </div>
        <span className="text-[11px] sm:text-xs text-slate-400 hidden sm:inline">Track who finished which problem</span>
      </div>
        ...
        <table className="w-full text-left border-collapse min-w-[480px]">
          <thead>
            <tr className="border-b border-slate-800 text-[10px] sm:text-[11px] text-slate-400 uppercase tracking-wider">
              <th className="py-2.5 sm:py-3 px-2 sm:px-3 min-w-[130px]">Member</th>
              {problems.map((prob) => (
                <th key={prob.id} className="py-2.5 sm:py-3 px-2 sm:px-3 text-center min-w-[100px]">
            ...
            {activeRoom.members.map((member) => (
              <tr key={member.id} className="hover:bg-slate-800/40 transition-colors">
                ...
                {problems.map((prob) => {
                  ...
                  return (
                    <td key={prob.id} className="py-2.5 sm:py-3 px-2 sm:px-3 text-center">
                      {isSolved ? (
