...
export const DailyProblemHero: React.FC<DailyProblemHeroProps> = ({ problem }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="glass-panel bg-gradient-to-br from-slate-900/90 via-slate-900/60 to-emerald-950/30 border border-slate-800 rounded-2xl p-6 relative overflow-hidden shadow-xl">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-10">
          <div>
            <div className="flex items-center gap-2 flex-wrap mb-2">
              <span className="bg-emerald-500/20 text-emerald-300 text-[10px] uppercase font-bold tracking-wider px-2.5 py-1 rounded-md border border-emerald-500/30 flex items-center gap-1">
                <Flame className="w-3 h-3 text-emerald-400" /> Daily Challenge
              </span>
              <span className={`text-xs font-semibold px-2.5 py-0.5 rounded-full border ${getDiffBadge(problem.difficulty)}`}>
                {problem.difficulty}
              </span>
              {problem.targetTimeMinutes && (
                <span className="text-xs text-slate-400 flex items-center gap-1">
                  <Clock className="w-3.5 h-3.5" /> {problem.targetTimeMinutes} mins
                </span>
              )}
            </div>
            ...
            <h2 className="text-2xl font-extrabold text-white tracking-tight flex items-center gap-3">
              {problem.title}
              {isSolved && (
                <span className="bg-emerald-500/20 text-emerald-400 text-xs px-2.5 py-1 rounded-full border border-emerald-500/40 flex items-center gap-1 font-semibold">
            ...
            <div className="flex items-center gap-2 flex-wrap mt-3">
              {problem.tags.map((tag, i) => (
                <span key={i} className="bg-slate-800/80 text-slate-300 text-xs px-2.5 py-1 rounded-lg border border-slate-700/60 flex items-center gap-1">
            ...
            <div className="text-xs text-slate-400 mt-3 flex items-center gap-1.5">
              <span>Posted by:</span>
              <img src={problem.postedBy.avatar} alt="" className="w-4 h-4 rounded-full object-cover" />
              <span className="text-slate-200 font-medium">{problem.postedBy.name}</span>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-2.5 shrink-0">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="bg-slate-800 hover:bg-slate-700 text-white text-xs px-4 py-2.5 rounded-xl font-semibold border border-slate-700 flex items-center justify-center gap-2 transition-all"
            >
              Solve on LeetCode
            ...
            {!isSolved ? (
              <button
                ...
                className="bg-emerald-500 hover:bg-emerald-400 text-slate-950 text-xs px-5 py-2.5 rounded-xl font-bold flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/20 transition-all glow-emerald"
              >
              ...
              <button
                onClick={() => setIsSubmitOpen(true)}
                className="bg-slate-800/80 hover:bg-slate-800 text-emerald-400 border border-emerald-500/30 text-xs px-4 py-2.5 rounded-xl font-semibold flex items-center justify-center gap-2 transition-all"
              >
